...
auto read_graph_from_file() {
	...
	/*
	c : 주석 -> 무시
	p sp N M : 문제를 나타내는 행 sp = Shortest Path / N = 정점 수 / M = 에지 수
	a U V W : 에지(방향)를 의미 U -> V / 가중치 = W
	*/
	...
	Graph<T> G(num_vertices + 1); // 왜 +1이지
	...
	while (getline(infile, line))
	{
		if (line[0] == 'a')
		{
			...
			T weight;
			iss >> p >> source_vertex >> dest_vertex >> weight; // p는 필요 없는 문자(a)
...
int main()
{
	...
	cout << endl << "거치는 정점 개수(시작과 끝 포함) : " << shortest_path.size();
}
