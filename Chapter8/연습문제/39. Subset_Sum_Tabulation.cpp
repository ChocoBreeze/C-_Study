...
#define DEBUG 0 // 전 과정 표시

// Memoization = 하향식 접근 방법

/*
원소 합이 6076인 부분집합이 있습니다.
BRUTE FORCE 방법 경과 시간: 8.56200

원소 합이 6076인 부분집합이 있습니다.
BACKTRACKING 방법 경과 시간: 0.00600

원소 합이 6076인 부분집합이 있습니다.
MEMOIZATION 방법 경과 시간: 0.00600

원소 합이 6076인 부분집합이 있습니다.
TABULATION 방법 경과 시간: 2.19400
...
bool SubsetSum_Memoization(vector<int>& set, int sum, int i, vector<vector<int>>& memo) {
	//memo = 캐시
	// 만약 현재 부분집합의 합이 target과 같다면 -- 기저 조건 체크
	if (sum == 0)
	{
		return true;
	}

	// 만약 현재 부분집합의 합이 target보다 크거나, 집합의 끝에 도달했다면 -- 기저 조건 체크
	if (i == set.size() || set[i] > sum)
	{
		return false;
	}
...
vector<vector<bool>> SubsetSum_Tabulation(vector<int>& set) {
	...
	// 첫 차원 크기 = set의 크기 +1개, 두 번째 차원 크기 = set의 모든 합보다 1크게(가능한 합 = 0~maxSum).
	vector<vector<bool>> DP(set.size() + 1, vector<bool>(maxSum + 1, 0));

	// 부분집합의 합이 0인 기저 조건에 대해서는 true로 설정.
	for (int i = 0; i < set.size(); i++)
	{
		DP[i][0] = true;
	}
