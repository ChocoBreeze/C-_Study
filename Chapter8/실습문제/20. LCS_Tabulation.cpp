...
int LCS_Memoization(string A, string B, int i, int j, vector<vector<int>>& memo) { // 따로 저장은 안 함 - 길이 존재 여부만..
...
string LCS_Tabulation(string A, string B) {
	// 구하는 과정
	vector<vector<int>> DP(A.size() + 1, vector<int>(B.size() + 1)); // 다 0으로 초기화
