...
class SimpleDisjointSet { // Disjoint set 자료구조
private:
	struct Node {
		unsigned id;
		unsigned rank;
		...
		Node(unsigned _id) : id(_id), rank(0), parent(_id) {} // 초기 : 부모 = 자기 자신 가리킴
...
public:
	...
	unsigned find(unsigned x) { // tree의 루트를 반환
	...
	void union_sets(unsigned x, unsigned y) // 두 트리 합치기
	{
		auto root_x = find(x);
		auto root_y = find(y);
		...
		// 작은 랭크의 트리를 큰 랭크의 트리 쪽으로 병합 -- 작은 랭크의 트리가 루트가 되도록
...
template <typename T>
class Graph {
	// 처음 정점 개수 설정 시 이후 변경 불가능
	// 에지 추가는 구현되어 있으나 에지 삭제는 미구현
public:
	// N개의 정점으로 구성된 그래프
...
Graph<T> minimum_spanning_tree(const Graph<T>& G) {
	...
	for (unsigned i = 0; i < N; i++) // N개 만들고 번호 넣기
		dset.make_set(i);
