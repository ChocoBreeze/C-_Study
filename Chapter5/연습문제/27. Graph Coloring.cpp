...
auto greedy_coloring(const Graph<T>& G)
{
	...
	vector<unsigned> assigned_colors(size); // 정점 개수 만큼

	// 1번 정점부터 차례대로 검사합니다.
	for (unsigned i = 1; i < size; i++)
	{
		auto outgoing_edges = G.edges(i); // i에서 나가는 모든 edge 반환

		set<unsigned> neighbours; // i번째 정점과 인접해있는 정점들의 현재 색상
		...
		{	
			// set.find -> element의 iterator 반환 못 찾으면 .end() 반환
			// set.end -> Reture iterator to end
