...
auto find_median(typename std::vector<T>::iterator begin, typename std::vector<T>::iterator last) {
	...
	return begin + (std::distance(begin, last) / 2); // median 원소 반복자 반환
}
...
{
	// 피벗 위치는 함수 인자로 주어지므로, 벡터의 시작과 마지막 원소를 가리키는 반복자를 정의합니다.
	...
	size_t i)
{
	...
	if (size > 0 && i < size)
	{
		...
		auto num_Vi = (size + 4) / 5; // 나머지 까지 고려한 개수 계산
		size_t j = 0;
		...
		for (; j < size / 5; j++)
		{
			auto b = begin + (j * 5);			// 나눠진 Vi의 시작
			auto l = begin + (j * 5) + 5;		// 나눠진 Vi의 끝

			M.push_back(*find_median<T>(b, l));
		}

		if (j * 5 < size)		// 5개 다 못 채운 마지막 vector 계산
		{
			auto b = begin + (j * 5);
